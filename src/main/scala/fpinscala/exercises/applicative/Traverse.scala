...
import fpinscala.answers.applicative.Tree
...
trait Traverse[F[_]] extends Functor[F], Foldable[F]:
  ...
  extension [A](fa: F[A])
    def map[B](f: A => B): F[B] =
      import Traverse.idTraverse
      fa.traverse[Id, B] { a =>
        Id(f(a))
      }(using Applicative.IdStuff.idApplicative).value

    override def foldMap[B](f: A => B)(using mb: Monoid[B]): B =
      fa.traverse[Const[B, _], Nothing](f)

    override def foldLeft[B](acc: B)(f: (B, A) => B): B =
      fa.mapAccum(acc){ case (a, s) =>
        // unclear why I couldn't do this the other way
        val b = f.curried(s)(a)
        fa.map(_ => b) -> b
      }(1)

    override def toList: List[A] =
      fa.foldLeft(List.empty[A]) { case (acc, a) =>
        a :: acc
      }
    ...
    def fuse[M[_], N[_], B](f: A => M[B], g: A => N[B])(using m: Applicative[M], n: Applicative[N]): (M[F[B]], N[F[B]]) =
      fa.traverse[[x] =>> (M[x], N[x]), B](x => (f(x), g(x)))(using m.product(n))
  ...
  given listTraverse: Traverse[List] with
    extension [A](as: List[A])
      override def traverse[G[_]: Applicative, B](f: A => G[B]): G[List[B]] =
        ...
        as.foldRight(g.unit(List.empty[B])) { case (a, acc) =>
          val gb: G[B] = f(a)
          gb.map2(acc) { case (b, list) =>
            b :: list
          }
        }
  ...
  given optionTraverse: Traverse[Option] with
    extension [A](oa: Option[A])
      override def traverse[G[_]: Applicative, B](f: A => G[B]): G[Option[B]] =
        val g = summon[Applicative[G]]
        oa.foldRight(g.unit(None:Option[B])) { case (a, acc) =>
          val gb: G[B] = f(a)
          gb.map(Some(_))
        }
  ...
  given mapTraverse[K]: Traverse[Map[K, _]] with
    extension [A](m: Map[K, A])
      override def traverse[G[_]: Applicative, B](f: A => G[B]): G[Map[K, B]] =
        val g = summon[Applicative[G]]
        m.foldRight(g.unit(Map.empty[K, B])) { case ((k, v), acc) =>
          val gb: G[B] = f(v)
          gb.map2(acc) { case (b, map) =>
            map.updated(k, b)
          }
        }
