...
object MyProgram:
  ...
  def fib(n: Int): Int =
    ...
    def go(n: Int, acc: List[Int]): Int =
      if n < acc.length then acc(n)
      else go(n, acc :+ acc.takeRight(2).sum)
    go(n, List(0,1))
...
object PolymorphicFunctions:
  ...
  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean = {
    as.sliding(2).forall { pair =>
      // construct this way to handle equality being sorted
      if pair.length == 2 then !gt(pair(0), pair(1))
      else true
    }
  }
